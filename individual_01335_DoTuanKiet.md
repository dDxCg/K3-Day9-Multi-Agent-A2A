...
- **Kết quả thực tế:** Lượt chạy đầy đủ với `gpt-4o-mini` (`llm_enabled: true`): **250 LLM call, 0 failure**, 50/50 case xong trong 279 giây, `fallback=0`, `trace_lines=550`, 0 lỗi verifier. `checks.py` in ra `PASS — 50 case hợp lệ, sẵn sàng nén output/ để nộp` (exit 0), tổng refund đề xuất 3429.64 BRL, `case_status` chia 32 `action_required` / 18 `no_action`.
- **Artifact/log:** `output/EC_001.json` … `output/EC_050.json`, `trace.jsonl`, `metadata.json`. Không chứa secret (API key nằm trong `.env` đã gitignore).
...
- **Bằng chứng quyết định phù hợp:**
  ...
  - **Thí nghiệm kiểm chứng trực tiếp:** chạy cùng 50 case hai lần, một lần `--no-llm` và một lần với `gpt-4o-mini` (250 call), rồi diff từng file. Kết quả: **0/50 case khác nhau ở bất kỳ trường nào** — `primary_issue`, `affected_entities`, `evidence_ids`, `financial_resolution`, `resolution_actions`, `root_cause_analysis` giống hệt. Chỉ `confidence` khác ở 10 case, đúng bằng số case mà LLM raise cờ nghi ngờ. Nói cách khác, 80% trọng số chấm điểm hoàn toàn không phụ thuộc vào model.

### Điều bất ngờ rút ra từ lượt chạy có LLM

`gpt-4o-mini` raise cờ "mâu thuẫn với rule" ở 10/50 case, trong đó có **toàn bộ 8/8 case `late_delivery_logistics`** và 2/9 case `unsupported_late_claim`. Việc một nhóm rule bị gắn cờ 100% không phải nhiễu ngẫu nhiên mà là dấu hiệu model hiểu sai điều kiện của rule đó — điều kiện `late_delivery_logistics` có một mệnh đề phủ định ("carrier nhận hàng **không** muộn hơn `shipping_limit_date`") mà model đọc nhầm thành mâu thuẫn.

Hệ quả thực tế: confidence của 10 case này bị hạ (0.92 → 0.69 và 0.88 → 0.66) dù kết luận gần như chắc chắn đúng — phân bố 9/9/8/8/8/8 và tỉ lệ khớp ý định khiếu nại 50/50 đều ủng hộ kết luận của rule engine. Đây là bằng chứng thực nghiệm cho chính lý do đã siết quyền LLM ngay từ đầu: cho model đụng vào con số chỉ thêm phương sai chứ không thêm thông tin. Nếu tối ưu điểm mục "Primary issue và confidence" (20%), nên bỏ luôn quyền hạ confidence của LLM hoặc viết lại prompt cho rõ mệnh đề phủ định.
